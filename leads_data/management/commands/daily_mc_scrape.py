...
target_table = driver.find_element(By.XPATH, "//a[@name='CPL']/following::table[1]")
    ...
    for mc_number in mc_list:
        ...
        try:
            ...
            for row in rows:
                ...
                owned = int(row.find_elements(By.TAG_NAME, "td")[0].text)
                term_leased = int(row.find_elements(By.TAG_NAME, "td")[1].text)
                trip_leased = int(row.find_elements(By.TAG_NAME, "td")[2].text)
        ...
        if loop_counter%10 == 0 and len(data) != 0:
            ...
            write_to_excel(data, f'mc_data_{range_set_index}.xlsx')
            data.clear()
...
#     if filename.startswith('mc_data_'):
...
# output_file = datetime.today().strftime('%Y-%m-%d') + '.xlsx'

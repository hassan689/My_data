...
def process_single_imap_account(profile_id):
    try:
        ...
        if not imap_conn: return f"Failed: {account.email_address} (Conn)"
        
        today = timezone.now().date()
        
        try:
            # 1. Rescue from Spam
            rescued = rescue_from_spam(imap_conn)
            if rescued > 0:
                DailyStat.objects.filter(profile=profile, date=today).update(inbox=F('inbox') + rescued)
